...
const client = new Highrise(config.highriseUrl, config.highriseToken)

export default function sync (lastCheck, cb) {
  client.get('recordings.xml', {since: lastCheck}, function (err, data) {
    ...
    if (!data.length) {
      debug('No new activity in Highrise since ' + lastCheck)
      return cb(null, lastCheck)
    }
...
function formatWebhook (recording, cb) {
  ...
  parseBody(recording, function (err, body) {
    ...
    const recordingLink = config.highriseUrl + recording.type + 's/' + recording.id
    const subjectLink = config.highriseUrl + getSubjectPath(recording.subjectType) + '/' + recording.subject.id
