...
module.exports = function sync (lastCheck, cb) {
  client.get('recordings.xml', {since: lastCheck}, function (err, data) {
    ...
    debug('found %d new recordings', data.length)

    var filteredData = data
      .filter(filterRecord)
      .sort(cmp('createdAt'))

    if (!filteredData.length) return cb(null, lastCheck)
  ...
  function filterRecord (record) {
    ...
    return (record.type === 'email' || record.type === 'note' || record.type === 'comment') &&
...
function sendWebhook (recording, cb) {
  ...
  client.get(getSubjectPath(recording.subjectType) + '/' + recording.subjectId + '.xml', function (err, contact) {
    if (err) return done(err)
...
function formatWebhook (recording, cb) {
  ...
  var recordingType = recordingDesc[recording.type] || 'a note'
